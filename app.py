...
class Venue(db.Model):
    ...
    shows = db.relationship('Show', backref='svenue', lazy='dynamic')
...
class Artist(db.Model):
    ...
    def __repr__(self):
      return f'<Artist {self.id}, Artist {self.name}>'
...
class Show(db.Model):
    ...
    def __repr__(self):
      return f'< Artist {self.artist_id}, Venue {self.venue_id} , Show {self.start_time}>'
...
def venues():
  ...
  # for each  city/state and add venues
  ...
  for venue in venues:
    ...
    # current date to filter num_upcoming_shows
    current_date = datetime.now()

    for show in shows:
      if show.start_time > format_datetime(str(current_date)):
...
def show_venue(venue_id):
  ...
  for show in shows:
    ...
    if show.start_time > format_datetime(str(current_time)):
...
def search_artists():
  ...
  search_term = request.form.get('search_term', '') #done
...
def show_artist(artist_id):
  ...
  for show in shows:
    data = {
          ...
          "venue_name": show.svenue.name,
          "venue_image_link": show.svenue.image_link,
          "start_time": format_datetime(str(show.start_time))
        }
    if show.start_time > format_datetime(str(current_time)):
      upcoming_shows.append(data)
...
def shows():
  ...
  #       num_shows should be aggregated based on number of upcoming shows per venue.
   #data=Show.query.all()
  shows = Show.query.order_by(Show.start_time)
  data = []
  for show in shows:
    data.append({
        "venue_id": show.venue_id,
